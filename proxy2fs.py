...
def response(context, flow):
    ...
    with decoded(flow.response):
        if (flow.response.status_code == 200):
            ...
            redomslash = r'(https?:)?(\/\/)?(\\\/\\\/)?(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.(com|org|edu|gov|uk|net|ca|de|jp|fr|au|us|ru|ch|it|nl|se|no|es|mil)(:[0-9]+)?\/\b'
            redom = r'(https?:)?(\/\/)?(\\\/\\\/)?(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.(com|org|edu|gov|uk|net|ca|de|jp|fr|au|us|ru|ch|it|nl|se|no|es|mil)(:[0-9]+)?\b'
            
            filedata = flow.response.content
            if data['filetype'] == "text/html":
                filedata = re.sub(redomslash, '/', filedata)
                filedata = re.sub(redom, '/', filedata)
            if data['filetype'] == "text/javascript":
                filedata = re.sub(redomslash, '/', filedata)
                filedata = re.sub(redom, '/', filedata)
            if data['filetype'] == "application/javascript":
                filedata = re.sub(redomslash, '/', filedata)
                filedata = re.sub(redom, '/', filedata)
            if data['filetype'] == "text/css":
                filedata = re.sub(redomslash, '/', filedata)
                filedata = re.sub(redom, '/', filedata)

            filedata = re.sub(r'\.aspx', '.php', filedata)
            filepath = filepath.replace('.aspx', '.php')
